...
import {Activity,BarChart3,CalendarDays,ChevronDown,Download,PackageOpen,Radio,RefreshCw,Search,ShoppingBag,Zap} from 'lucide-react';
...
type MetaCampaign={campaign:{id:string;name:string;status:string;active:boolean};totals:{spend:number;purchases:number;revenue:number;cpa:number;roas:number};creatives:{id:string;name:string;spend:number;purchases:number;revenue:number;cpa:number;roas:number}[];syncedAt:string;source:string};
...
function ActiveCampaignAlert(){
 const [data,setData]=useState<MetaCampaign|null>(null),[error,setError]=useState(''),[loading,setLoading]=useState(true);
 const load=useCallback(async()=>{setLoading(true);try{const response=await apiFetch('/api/meta-campaign');const body=await response.json();if(!response.ok)throw new Error(body.error);setData(body);setError('')}catch(error){setError(error instanceof Error&&error.message==='meta_not_configured'?'Conecte o token do Meta na Vercel para ativar este alerta.':'Não foi possível atualizar a campanha do Meta.')}finally{setLoading(false)}},[]);
 useEffect(()=>{load();const timer=window.setInterval(load,300000);return()=>window.clearInterval(timer)},[load]);
 if(error&&!data)return <div className="finance-warning">{error}</div>;
 if(!data)return <section className="campaign-alert loading"><RefreshCw className="spin"/><span>Buscando a campanha ativa no Meta…</span></section>;
 return <section className="campaign-alert"><div className="campaign-alert-head"><div className="campaign-title"><span className={data.campaign.active?'live-dot':'live-dot off'}/><div><small>{data.campaign.active?'CAMPANHA ATIVA':'CAMPANHA PAUSADA'}</small><h2>{data.campaign.name}</h2><p>Hoje · dados atribuídos pelo Meta</p></div></div><button onClick={load} disabled={loading}><RefreshCw className={loading?'spin':''}/>Atualizar</button></div><div className="campaign-totals"><div><span>Investimento</span><strong>{money(data.totals.spend)}</strong></div><div><span>Vendas</span><strong>{data.totals.purchases}</strong></div><div><span>Faturamento</span><strong>{money(data.totals.revenue)}</strong></div><div><span>CPA</span><strong>{money(data.totals.cpa)}</strong></div><div><span>ROAS</span><strong>{data.totals.roas.toFixed(2).replace('.',',')}x</strong></div></div><div className="campaign-creatives"><div className="campaign-creative labels"><span>Criativo</span><span>Vendas</span><span>Faturamento</span><span>CPA</span><span>ROAS</span></div>{data.creatives.length?data.creatives.map(row=><div className="campaign-creative" key={row.id}><b>{row.name}</b><span>{row.purchases}</span><span>{money(row.revenue)}</span><span>{money(row.cpa)}</span><span>{row.roas.toFixed(2).replace('.',',')}x</span></div>):<p className="campaign-empty">Nenhuma entrega registrada hoje.</p>}</div><footer><Zap/> Atualização automática a cada 5 minutos · {new Date(data.syncedAt).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</footer></section>;
}
