...
export default async function handler(req:any,res:any){
 if(req.method==='OPTIONS'){allowOrigin(req,res);return res.status(204).end()} if(req.method!=='POST')return json(res,405,{error:'method_not_allowed'}); if(!allowOrigin(req,res))return json(res,403,{error:'origin_not_allowed'});
